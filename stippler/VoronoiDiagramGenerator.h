...
public:
	...
	bool getNext(float& x1, float& y1, float& x2, float& y2, float &s1x, float &s1y, float &s2x, float &s2y)
	{
		if(iteratorEdges == 0)
			return false;
		...
		y1 = iteratorEdges->y1;
		y2 = iteratorEdges->y2;
		s1x = iteratorEdges->s1x;
		s1y = iteratorEdges->s1y;
		s2x = iteratorEdges->s2x;
		s2y = iteratorEdges->s2y;
